...
def _(environment, **kw):
    ...
    if environment.stats.total.fail_ratio > max_failure_rate:
        logging.error(
            f"Test failed due to failure ratio > "
        ...
        logging.error(
            "Test failed due to average response time ratio > %d ms:" " %d ms",
            max_avg_response_time,
            environment.stats.total.avg_response_time,
        )
        environment.process_exit_code = 1
    elif (
        environment.stats.total.get_response_time_percentile(
            max_percentile_time[0]
        )
        > max_percentile_time[1]
    ):
        logging.error(
            "Test failed due to %dth percentile response time > %d " "ms",
            max_percentile_time[0],
            max_percentile_time[1],
        )
        environment.process_exit_code = 1
...
class UsiLoadTester(locust.HttpUser):
    ...
    def generate_json(self):
        usi = random.choice(usis_to_test)
        self.client.get(f"/json/?usi1={usi}", name="/json/")

    @locust.task
    def generate_csv(self):
        usi = random.choice(usis_to_test)
        self.client.get(f"/csv/?usi1={usi}", name="/csv/")
    ...
    def generate_mirror_png(self):
        ...
        usi2 = random.choice(usis_to_test)
        self.client.get(
            f"/png/mirror/?usi1={usi1}&usi2={usi2}", name="/png/mirror/"
        )
    ...
    def generate_mirror_svg(self):
        ...
        usi2 = random.choice(usis_to_test)
        self.client.get(
            f"/svg/mirror/?usi1={usi1}&usi2={usi2}", name="/svg/mirror/"
        )
    ...
    def generate_mirror_json(self):
        ...
        usi2 = random.choice(usis_to_test)
        self.client.get(
            f"/json/mirror/?usi1={usi1}&usi2={usi2}", name="/json/mirror/"
        )
