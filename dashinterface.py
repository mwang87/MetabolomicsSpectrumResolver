...
CONTRIBUTORS_DASHBOARD = [
    ...
    dbc.CardBody(
        [
            ...
            html.H5("Citation"),
            html.A(
                "Bittremieux, Wout, Christopher Chen, Pieter C. "
...
def determine_task(pathname, search):
    ...
    annotate_precision = _get_url_param(
        query_dict, "annotate_precision", dash.no_update
    )
    annotation_rotation = _get_url_param(
        query_dict, "annotation_rotation", dash.no_update
    )
    cosine = _get_url_param(query_dict, "cosine", dash.no_update)
    fragment_mz_tolerance = _get_url_param(
        query_dict, "fragment_mz_tolerance", dash.no_update
    )
...
def _process_single_usi_table(usi, plotting_args):
    ...
    peaks_df = pd.DataFrame(
        {
            "mz": [peak[0] for peak in peaks_list],
    ...
    return (
        peaks_df.to_dict(orient="records"),
        columns,
...
def _process_mirror_usi(usi1, usi2, plotting_args):
    ...
    # FIXME
    spectrum1, spectrum2 = views._prepare_mirror_spectra(
        spectrum1, spectrum2, cleaned_plotting_args
    )
    ...
    download_div = dbc.Row(
        [
            dbc.Col(
                [
                    html.Img(
                        src=f"/qrcode?mirror=true&usi1={quote(usi1)}&"
                        f"usi2={quote(usi2)}"
                    )
                ],
                className="col-1",
